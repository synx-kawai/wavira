...
static esp_err_t wifi_ping_router_start(ip_event_got_ip_t* event);
                                ...
                                int32_t event_id, void* event_data)
{
    ...
    ESP_LOGI(TAG, "Initializing CSI and starting Ping...");
    wifi_csi_init();
    wifi_ping_router_start(event);
    
    // xSemaphoreGive(csi_start_semaphore); // No longer needed
}
...
static esp_err_t wifi_ping_router_start(ip_event_got_ip_t* event)
{
    ...
    ping_config.target_addr.u_addr.ip4.addr = event->ip_info.gw.addr;
...
void app_main()
{
    ...
    ESP_LOGI(TAG, "Waiting for Wi-Fi connection and IP address...");
}
